...
{  if not wnd_ready then
    ...
    WM_ACTIVATEAPP :
      ...
      // Активация / Деактивация окна
        with ownd do
          ...
          begin
            wnd_active := False;
            if not wnd_mode then Show(True);
            if not wnd_mode then
            begin
  ...
  // клавиатура
    WM_KEYUP   : oinp.SetKey(wParam, 0);
    WM_KEYDOWN : oinp.SetKey(wParam, 1);
  // мышь
    WM_LBUTTONUP   : oinp.SetKey(M_BTN_L, 0);
    WM_RBUTTONUP   : oinp.SetKey(M_BTN_R, 0);
    WM_MBUTTONUP   : oinp.SetKey(M_BTN_M, 0);
    WM_LBUTTONDOWN : oinp.SetKey(M_BTN_L, 1);
    WM_RBUTTONDOWN : oinp.SetKey(M_BTN_R, 1);
    WM_MBUTTONDOWN : oinp.SetKey(M_BTN_M, 1);
...
procedure TWnd.log(Text: string);
begin
...
// изменение стиля окна в зависимости от режима работы
  ...
  begin
    ...
    with Rect do
      MoveWindow(wnd_handle, 0, 0, Right - Left, Bottom - Top, False);
    ShowWindow(wnd_handle, SW_SHOW);
  end;
