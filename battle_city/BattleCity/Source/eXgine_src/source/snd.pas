...
// если не нашли - дублируем буфер в новый "полностью свободный" канал
  ...
  if not Channels[NID].Sample then
    Channels[NID].Ref := 1;
  Channels[NID].Pos := ovec.Create(X, Y, Z);
...
begin
  if (not snd_ready) or
     ...
     (Channels[ID].Buffer = nil) then
    Exit;
  Channels[ID].Pos := ovec.Create(X, Y, Z);
  Calc(ID);
end;
...
procedure TSnd.log(Text: string);
begin
